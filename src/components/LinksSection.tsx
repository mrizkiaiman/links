...
export function LinksSection({ theme }: LinksSectionProps) {
  ...
  return (
    <section key="links" className="space-y-4">
      {links.map((link) => (
        <a
          key={link.id}
          href={link.url}
          ...
          className={`group relative block rounded-[2rem] overflow-hidden border transition-all active:scale-[0.98] ${isDarkMode ? "bg-gradient-to-br from-white/10 to-white/5 border-white/10" : "bg-white border-gray-100 shadow-sm"}`}
        >
